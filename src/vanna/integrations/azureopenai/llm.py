...
Azure OpenAI's Chat Completions API. It uses the AzureOpenAI client from the
openai package with Azure-specific configuration.
"""
...
class AzureOpenAILlmService(LlmService):
    ...
    This service uses the AzureOpenAI client from the openai package to connect
    to Azure OpenAI endpoints. It requires Azure-specific parameters: endpoint,
    deployment name, and API version.

    Args:
        model: Azure OpenAI deployment name (e.g., "gpt-4", "gpt-35-turbo").
            Falls back to env `AZURE_OPENAI_MODEL` or defaults to "gpt-4".
        api_key: Azure OpenAI API key; falls back to env `AZURE_OPENAI_API_KEY`.
        api_version: Azure OpenAI API version (e.g., "2024-02-15-preview");
            falls back to env `AZURE_OPENAI_API_VERSION` or defaults to "2024-02-15-preview".
        azure_endpoint: Azure OpenAI endpoint URL (e.g., "https://your-resource.openai.azure.com");
            falls back to env `AZURE_OPENAI_ENDPOINT`.
        extra_client_kwargs: Extra kwargs forwarded to `openai.AzureOpenAI()`.
    """
    ...
    def __init__(
        ...
        **extra_client_kwargs: Any,
        ...
        # Azure OpenAI requires endpoint, API key, and API version
        azure_endpoint = azure_endpoint or os.getenv("AZURE_OPENAI_ENDPOINT")
        if not azure_endpoint:
            raise ValueError(
                "azure_endpoint is required. Set it explicitly or via AZURE_OPENAI_ENDPOINT env var."
            )
        ...
        if not api_key:
            raise ValueError(
                "api_key is required. Set it explicitly or via AZURE_OPENAI_API_KEY env var."
            )
